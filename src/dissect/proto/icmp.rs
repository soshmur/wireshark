...
pub fn dissect(data: &[u8], ctx: &mut Ctx) -> Result<Node> {
    ...
    let mut node = Node::new("icmp", start..start + data.len(), Value::None)
        .with_source(s)
        .reserve(8);
    node.push(Node::new("icmp.type", ty_r, Value::Unsigned(u64::from(ty))).with_source(s));
    node.push(Node::new("icmp.code", code_r, Value::Unsigned(u64::from(code))).with_source(s));
    node.push(
        Node::new(
            "icmp.checksum",
            checksum_r.clone(),
    ...
    node.push(
        Node::new("icmp.checksum.status", checksum_r, Value::Unsigned(status)).with_source(s),
    );
    ...
    match ty {
        0 | 8 | 13 | 14 => {
            ...
            let (seq, seq_r) = c.u16()?;
            node.push(
                Node::new("icmp.ident", ident_r, Value::Unsigned(u64::from(ident))).with_source(s),
            );
            node.push(Node::new("icmp.seq", seq_r, Value::Unsigned(u64::from(seq))).with_source(s));
            ctx.set_info(format!("{type_name} id=0x{ident:04x}, seq={seq}"));
            if !c.is_empty() {
                node.push(Node::new("icmp.data", c.rest_range(), Value::Bytes).with_source(s));
            }
        }
        3 | 4 | 11 | 12 | 5 => {
            match ty {
                3 if code == 4 => {
                    ...
                    let (mtu, mtu_r) = c.u16()?;
                    node.push(
                        Node::new("icmp.unused", unused_r, Value::Unsigned(0)).with_source(s),
                    );
                    node.push(
                        Node::new("icmp.mtu", mtu_r, Value::Unsigned(u64::from(mtu)))
                            .with_source(s),
                    );
                }
                5 => {
                    let (gw, gw_r) = c.ipv4()?;
                    node.push(Node::new("icmp.gateway", gw_r, Value::Ipv4(gw)).with_source(s));
                }
                12 => {
                    let (ptr, ptr_r) = c.u8()?;
                    let (_, unused_r) = c.take(3)?;
                    node.push(
                        Node::new("icmp.pointer", ptr_r, Value::Unsigned(u64::from(ptr)))
                            .with_source(s),
                    );
                    node.push(
                        Node::new("icmp.unused", unused_r, Value::Unsigned(0)).with_source(s),
                ...
                _ => {
                    ...
                    node.push(
                        Node::new("icmp.unused", unused_r, Value::Unsigned(u64::from(unused)))
                            .with_source(s),
                    );
                }
            }
            ...
            if !c.is_empty() {
                let saved = (ctx.summary.clone(), ctx.net_addrs, ctx.protocols.len());
                let inner = nested_ipv4(c.rest(), c.abs(), ctx);
                ctx.summary = saved.0;
                ctx.net_addrs = saved.1;
                ctx.protocols.truncate(saved.2);
                for n in inner {
                    node.push(n);
                }
            }
...
pub fn nested_ipv4(data: &[u8], base: usize, ctx: &mut Ctx) -> Vec<Node> {
    let mut out = Vec::new();
    if ctx.nesting >= 2 {
        return out;
    }
    ...
    for _ in 0..4 {
        ...
        match result {
            ...
            Err(e) => {
                out.push(
                    Node::new("_ws.malformed", offset..end, Value::None)
